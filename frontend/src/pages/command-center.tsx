...
export function OverviewPage() {
  ...
  return <div className="page command-page">
    ...
    <div className="econ-grid"><Metric label="Modeled baseline" value={usd(s.baselineCost)} detail="Counterfactual reference cost" /><Metric label="Optimized provider cost" value={usd(s.optimizedProviderCost)} detail="Paid directly by you" /><Metric label="Orchestra platform fee" value={usd(s.platformFee)} detail="Fee on positive modeled savings" /><Metric label="Customer final cost" value={usd(s.customerFinalCost)} detail="Provider cost + platform fee" tone={s.customerNetSavings !== null && s.customerNetSavings > 0 ? 'positive' : ''} /></div>
...
export function SavingsPage() {
  ...
  return <div className="page command-page">
    ...
    <section className="savings-hero"><div><span className="eyebrow">NET CUSTOMER SAVINGS</span><h1>{state === 'saved' ? usd(s.customerNetSavings) : stateLabel(state)}</h1><p>{state === 'saved' ? 'After the Orchestra platform fee.' : state === 'cost_increase' ? 'The selected data cost more after optimization and the platform fee.' : 'The selected data contains no positive verified modeled savings.'}</p></div><div className="hero-stat"><span>SAVINGS RATE</span><b>{fmtPct(s.savingsRate || 0)}</b></div><div className="hero-stat"><span>RUNS</span><b>{s.runCount || 0}</b></div></section>
