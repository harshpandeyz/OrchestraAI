...
function validateCommandArgs(bin, args, workspace) {
  if (args.length > 20) throw Object.assign(new Error('too many command arguments'), { code: 'denied' });
  const ws = (() => { try { return resolveWorkspaceRoot(workspace); } catch { return null; } })();
  // Brackets are allowed (pytest `test.py::case[param]` selectors); escaping
  // the workspace is still blocked by the path rules below. Control
  // characters (newline/NUL) are rejected by the explicit check above.
  const SHELL_META = /[;&|$`(){}<>*?!#~]/;
  for (const raw of args) {
    const a = String(raw);
    if (!a || a.length > 500) throw Object.assign(new Error('command argument too long'), { code: 'denied' });
    if (a.includes('\0') || a.includes('\n') || a.includes('\r')) {
      throw Object.assign(new Error('command argument contains control characters'), { code: 'denied' });
    }
    if (SHELL_META.test(a)) throw Object.assign(new Error(`command argument not allowed: ${a.slice(0, 60)}`), { code: 'denied' });
    if (/^[A-Za-z_][A-Za-z0-9_]*=/.test(a)) {
      throw Object.assign(new Error('environment assignments are not allowed in commands'), { code: 'denied' });
    }
    // Path-looking args must stay inside the workspace (blocks
    // `--rootdir=/etc`, `../../..`, absolute escapes).
    if (a.includes('/') || a.includes(path.sep)) {
      const segs = a.split(/[\\/]/);
      if (segs.includes('..')) throw Object.assign(new Error('path traversal is not allowed in commands'), { code: 'denied' });
      if (ws && path.isAbsolute(a)) {
        const abs = path.resolve(a);
        if (abs !== ws && !abs.startsWith(ws + path.sep)) {
          throw Object.assign(new Error('command path escapes workspace'), { code: 'denied' });
        }
      }
    }
  }
  const joined = args.join(' ');
  // Interpreter code-execution flags are only dangerous in flag position
  // (`python -c <code>`, `node -e <code>` run inline code instead of a
  // workspace file). Script arguments after the file are untouched.
  const first = args[0] || '';
  if (bin === 'node' && first.startsWith('-')) {
    throw Object.assign(new Error('node flags are not allowed; provide a workspace file path'), { code: 'denied' });
  }
  if (bin === 'python' && (first === '-c' || first === '--command')) {
    throw Object.assign(new Error('inline code execution is not allowed via python'), { code: 'denied' });
  }
  // Flags that redirect tool execution outside the workspace.
  if (/(^|\s)(--prefix\b|--prefix=|--workspace\b|--workspace=|--workspaces\b|--node-options[= ]|--exec\b|--rootdir[= ])/.test(` ${joined}`)) {
    throw Object.assign(new Error('workspace-redirecting flags are not allowed'), { code: 'denied' });
  }
}
